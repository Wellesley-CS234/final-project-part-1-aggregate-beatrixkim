...
#data_path = "..\\data\\st13_data.csv" # redo so it is the session data

if 'student_data' not in st.session_state or st.session_state['student_data']['st13_df'].empty:
    st.warning("Data not loaded. Please ensure the main Home Page ran successfully and the data files exist.")
else:
    df=pd.read_csv("data/st13_data.csv")
    # convert date
